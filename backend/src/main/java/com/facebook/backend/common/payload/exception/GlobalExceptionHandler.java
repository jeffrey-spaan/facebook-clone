...
class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Handles {@link DataIntegrityViolationException} and returns a ProblemDetail object
     * with HTTP status 409 (Conflict) and a localized error message.
     *
     * @param ex the DataIntegrityViolationException thrown when there is a data integrity violation
     * @return a {@link ResponseEntity} object containing a {@code ProblemDetail} object with a detailed error message
     *        and an HTTP 409 Conflict status
     */
    ...
    ProblemDetail handleDataIntegrityViolationException(DataIntegrityViolationException ex) {
        String message = i18nService.getMessage(
                "global.exception.data.integrity.violation"
        );
